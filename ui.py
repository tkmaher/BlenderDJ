...
class OBJECT_MT_subMenu(bpy.types.Menu):
    ...
    def draw(self, context):
        ...
        #layout.operator("wm.low", text="Bind to 'Low level'")
        #layout.operator("wm.mid", text="Bind to 'Mid level'")
        #layout.operator("wm.high", text="Bind to 'High level'")
...
class high(bpy.types.Operator):
    bl_idname = "wm.high"
    bl_label= "high"
    
    def execute(self, context):
        listener.update("high")
        return {'FINISHED'}
'''
